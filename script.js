...
let openIsShop = false 
     8 < new Date().getHours()  ? openIsShop = true : openIsShop = false 

let order =  (time , work, Error) =>{
    return new Promise((resolve , reject)=>{
    openIsShop == true ? setTimeout(()=>{resolve(work())},time): reject(Error())})}

async function make(){
    try{
    await  order(2000, () => list.innerHTML = `<h3>Order accepted</h3>`)

    await  order(1500, () => list.innerHTML = `<h3 style="text-align: center;">Products has been prepared<h3>`) 
           
    await order(1500,()=>  list.innerHTML = `<h3>Relax with procces<h3>`)

    await order(2500,()=> list.innerHTML = `
